 ...
 * @class OCR
 */
function OCR(host, secretKey, ak, sk) {
  this.host = host;
...
OCR.fn.uploadFile = function uploadFile(opt) {
  ...
  // init data
  //var type = opt.type||'text';
  var path = '/TIMS-Server/postController/uploadFile.action';
  var url = opt.url;
  var httpMethod = 'post';
  ...
  return new Promise(function (resolve, reject) {
    ...
    getImg(url, '/tmp/xxdebug.jpg').then(function(filePath) {
       //var databuffer = new Buffer(JSON.stringify(data));
       //headers['Content-Type'] = 'claraapplication/json';
       //headers['Content-Length'] = databuffer.length;
